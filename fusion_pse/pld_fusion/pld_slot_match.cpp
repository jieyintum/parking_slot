    ...
    namespace PSE
    {
        ...
        void PldFusion::matchAlgo(PldFrame &frame, std::vector<FusionSlot::Ptr> &noMatchSlots)
        {
            ...
            while (false == PldFusion::isAllInfoofMatchedPSBeenProcessedFinished(frame.detectSlots_))
            {
                for (int i = 0; i < frame.detectSlots_.size(); i++)
                {
                    ...
                    if ((newSlot->matchedList.size() > 0) && (((*(newSlot->matchedList.end() - 1)).isDispatched == false) && (newSlot->lowestCostInfo.matchedSlot == nullptr)))
                    {
        ...
        bool PldFusion::isAllInfoofMatchedPSBeenProcessedFinished(const std::vector<FusionSlot::Ptr> detectSlots)
        {
            for (int itr = 0; itr < detectSlots.size(); itr++)
            {
                ...
                else if ((detectSlots[itr]->lowestCostInfo.matchedSlot == nullptr) && ((*(detectSlots[itr]->matchedList.end() - 1)).isDispatched == false))
                {
                    return false;
                }
            }
            ...
            const FusionSlot::Ptr slot)
        {
            ...
            {
                if (Fusion::isPointInsideSlot(trackSlot_->lines_, slot->middlePoint.worldCorner) || Fusion::isPointInsideSlot(slot->lines_, trackSlot_->middlePoint.worldCorner))
                {
                    return true;
                }
            }
            ...
            float &f_minOverlapRatio_f)
        {
            ...
            else
            {
                ...
                if ((0.5 > l_overlapRatioA_f) && (0.5 > l_overlapRatioB_f))
                {
                    return false;
                }
                else
            ...
            float &f_minOverlapRatio_f)
        {
            ...
            if ((0.8f > l_orthDistOfSP_f) && (0.8f > l_orthDistOfEP_f))
            {
