    ...
    const Odometry poseOb)
{
    ...
    for(auto& mergedSlot_ : mergedSlots.slotsList_) {
        ...
        if( (mergedSlot_->id == mergedSlots.targetSlotId)
         && (mergedSlots.targetSlotLoc != FusionSlot::SlotLoc::UNKNOWN)) {
    ...
    for(const auto& pldSlot_ : pldSlots) {
        ...
        if(pldSlot_->alignInfo.lastResultSlot != nullptr) {
            ...
            if(pldsPartnerSlot != nullptr) {
                //std::cout<<"11"<<"\n";
                if( (pldsPartnerSlot->alignInfo.lastPartnerSlot == nullptr)
                 && (pldsPartnerSlot->alignInfo.lastResultSlot != nullptr) ) {
                    //std::cout<<"12"<<"\n";
    ...
    const FusionSlot::Ptr slot)
{
    ...
    for(const auto& mergedSlot_ :  mergedSlotsList) {
        ...
        if( (Fusion::isPointInsideSlot(mergedSlot_->lines_, middlePoint))
         && (mergedSlot_->isUpdatedThisCircle == true) )
            ...
            const std::list<FusionSlot::Ptr> ussSlots)
{
    ...
    for(const auto& pldSlot_ : pldSlots) {
        if( (mergedSlots->alignInfo.pldSlot == pldSlot_)
         && (mergedSlots == pldSlot_->alignInfo.resultSlot) ) {
    ...
    for(const auto& ussSlot_ : ussSlots) {
        if( (mergedSlots->alignInfo.ussSlot == ussSlot_) 
         && (mergedSlots == ussSlot_->alignInfo.resultSlot) ) {
    ...
    const std::list<FusionSlot::Ptr> slotsListB)
{
    for(const auto & slotB_: slotsListB) {
        if( (slotA->alignInfo.lastPartnerSlot == slotB_) 
         && (slotB_->alignInfo.lastPartnerSlot == slotA) ) {
            return;
        }
    }
