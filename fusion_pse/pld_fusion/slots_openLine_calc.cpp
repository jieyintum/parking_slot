    ...
    namespace PSE
    {
            ...
            PldFrame &frame)
        {
            for (auto &tSlot : trackSlots)
            {
                ...
                if (std::fabs(std::acos(cosValue1) - std::acos(cosValue2)) < (double)(17.0f / 180.0f) * M_PI)
                {
                    ...
                    if ((sumAngle < (double)(70.0f / 180.0f) * M_PI) || (sumAngle > (double)(110.0f / 180.0f) * M_PI))
                    {
            ...
            for (auto &tSlot : trackSlots)
            {
                if ((tSlot->openLineIndex != 4) && (tSlot->openLineIndex != 0))
                {
        ...
        void PldFusion::calcWithFSPoint(const FusionSlot::Ptr slot, const Eigen::Vector3d fsPoint)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                if (isValuesInOrder(startTau, tau, endTau) && ((std::fabs(tau - startTau) / line->getLength() > 0.2f) && (std::fabs(endTau - tau) / line->getLength() > 0.2f)) && (line->calcOrthDistOfWorldPointAbs(fsPoint) < 1.5f))
                {
                    ...
                    {
                        if ((startConnectLine->calcOrthDistOfWorldPointAbs(fsPoint) > 0.17f * line->getLength()) && (startConnectLine->isPointOnLeftSideOfUnitVector(fsPoint)))
                        {
                    ...
                    {
                        if ((endConnectLine->calcOrthDistOfWorldPointAbs(fsPoint) > 0.17f * line->getLength()) && (endConnectLine->isPointOnLeftSideOfUnitVector(fsPoint)))
                        {
                            line->couldBeingAsOpenLine = Fusion::CTrackLine::OpenLineStatus::occupiedByFs;
                        }
                    }
        ...
        {
            if (std::fabs(Fusion::calAngleBetweenTwoUnitVector(lineA->getDefaultUnitVec(), lineB->getDefaultUnitVec())) < (float)(3 * M_PI / 4.0f))
            {
                return;
            }
            ...
            if (lineA->isPointOnLeftSideOfUnitVector(middleB) && oppositeLineA->isPointOnLeftSideOfUnitVector(middleB))
            {
                if ((lineA->calcOrthDistOfWorldPointAbs(middleB) > 1.5) && (oppositeLineA->calcOrthDistOfWorldPointAbs(middleB) > 1.5))
                {
                    return;
                }
            }

            if (lineB->isPointOnLeftSideOfUnitVector(middleA) && oppositeLineB->isPointOnLeftSideOfUnitVector(middleA))
            {
                if ((lineB->calcOrthDistOfWorldPointAbs(middleA) > 1.5) && (oppositeLineB->calcOrthDistOfWorldPointAbs(middleA) > 1.5))
                {
                    return;
                }
            }

            if ((lineA->calcOrthDistOfWorldPointAbs(lineB->getStartPoint()) < 2.2) && (lineA->calcOrthDistOfWorldPointAbs(lineB->getEndPoint()) < 2.2))
            {
            ...
            const FusionSlot::Ptr targetSlot)
        {
            ...
            if ((targetSlot->type == FusionSlot::Type::INCLINED) && (lines[0]->couldBeingAsOpenLine == Fusion::CTrackLine::OpenLineStatus::unknown) && (lines[1]->couldBeingAsOpenLine == Fusion::CTrackLine::OpenLineStatus::unknown))
            {
            ...
            if ((selectedLine != nullptr) && (targetSlot->block_.type != FusionSlot::BlockPoint::Type::NO_BLOCK))
            {
                ...
                if ((egoLine->isPointOnRightSideOfUnitVector(targetSlot->block_.worldPoint) && (!opposideLine->isPointOnRightSideOfUnitVector(targetSlot->block_.worldPoint))) || ((!egoLine->isPointOnRightSideOfUnitVector(targetSlot->block_.worldPoint)) && opposideLine->isPointOnRightSideOfUnitVector(targetSlot->block_.worldPoint)))
                {
                    if (egoLine->calcOrthDistOfWorldPointAbs(targetSlot->block_.worldPoint) > opposideLine->calcOrthDistOfWorldPointAbs(targetSlot->block_.worldPoint))
                    {
                        // do nothing
                        // std::cout<<"keep"<<"\n";
                    }
