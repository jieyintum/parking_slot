...
namespace Fusion
{
    namespace PSE
    {
        ...
        void PldFusion::Detect(PldFrame &frame)
        {
            ...
            {
                if ((PldFusion::slotAdjustUsingFs(*iter, frame.fs_) == false) && (isParking_ == false))
                {
        ...
        void PldFusion::ussSlotInputProcess(CUssSlotsList &ussSlots_, PldFrame &frame, const bool isInParkIn)
        {
            ...
            if ((ussSlots_.isLastFrameInParking == true) && (isInParkIn == false))
            {
                ussSlots_.reset();
            }
            ...
            const std::list<FusionSlot::Ptr> pldSlots)
        {
            ...
            for (int i = 0; i < fsStruct->outerObjects_.size(); i++)
            {
                for (int j = 0; j < fsStruct->outerObjects_[i].size(); j++)
                {
                    ...
                    {
                        if ((isPointInsideSlot(coreSlotPtr->lines_, fsStruct->outerObjects_[i][j].worldCorner)) || ((slot->lines_[0]->calcOrthDistOfWorldPointAbs(fsStruct->outerObjects_[i][j].worldCorner) < minDepth) && Fusion::isValuesInOrder(
                                                                                                                                                                                                                                tauThreshold0,
            ...
            for (int i = 0; i < fsStruct->innerObjects_.size(); i++)
            {
                for (int j = 0; j < fsStruct->innerObjects_[i].size(); j++)
                {
                    ...
                    {
                        if ((isPointInsideSlot(coreSlotPtr->lines_, fsStruct->innerObjects_[i][j].worldCorner)) || ((slot->lines_[0]->calcOrthDistOfWorldPointAbs(fsStruct->innerObjects_[i][j].worldCorner) < minDepth) && Fusion::isValuesInOrder(
                                                                                                                                                                                                                                tauThreshold0,
            ...
            for (auto &pldSlot : pldSlots)
            {
                ...
                {
                    if ((2.0f > Fusion::calcDistBetweenPoints(ussOpenLine->getStartPoint(), pldOpenLine->getStartPoint())) || (2.0f > Fusion::calcDistBetweenPoints(ussOpenLine->getStartPoint(), pldOpenLine->getEndPoint())) || (2.0f > Fusion::calcDistBetweenPoints(ussOpenLine->getEndPoint(), pldOpenLine->getStartPoint())) || (2.0f > Fusion::calcDistBetweenPoints(ussOpenLine->getEndPoint(), pldOpenLine->getEndPoint())))
                    {
                        return true;
                    }
                }
        ...
        bool PldFusion::slotAdjustUsingFs(const FusionSlot::Ptr slot, const FSStruct::Ptr fsStruct)
        {
            ...
            else if ((isP0WithInvade == false) && (isP1WithInvade == true))
            {
                newP1 = slot->lines_[0]->calcWorldPointUsingTauValue(tauOut1);
        ...
        {
            if (slot->lines_[0]->isPointOnLeftSideOfUnitVector(ObjworldPoint) && (slot->lines_[0]->calcOrthDistOfWorldPointAbs(ObjworldPoint) < minDepth))
            {
...
// ShowFusionPld(trackSlots_.slotsList_);
        ...
        void PldFusion::checkCornerConfidenceMatchedSlot(const FusionSlot::Ptr &fusionSlot)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                // }
                if ((fusionSlot->type == FusionSlot::Type::HORIZONTAL) && (isPointInsideSlot(vehicleRange->lines_, perceptSlot->corners[i].baseCorner) == true))
                {
                    perceptSlot->corners[i].confidence = 0.0f;
                }
            }
        ...
        void PldFusion::FusionMatchSlots(PldFrame &frame, std::list<FusionSlot::Ptr> &trackSlots)
        {
            ...
            for (auto &tSlot : trackSlots)
            {
                ...
                if (!isParking_)
                {
                    ...
                    {
                        if (isMoving_ && PldFusion::isInsideObserveRange(ObserveRange, tSlot))
                        {
                ...
                if (tSlot->updateActive)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if ((tSlot->relativeSlots.back()->corners[i].isCornerAsTruePoint == true) && ((tSlot->corners[i].isCornerAsTruePoint == true) || ((tSlot->corners[i].worldCorner - tSlot->relativeSlots.back()->corners[i].worldCorner).norm() < 1.0f)))
                        {
                            // as true point
        ...
        void PldFusion::EstimateOccupiedType(const FusionSlot::Ptr &trackSlot)
        {
            ...
            if ((occupiedNum > 5) && ((float)groundLockerCounter[0] - (float)groundLockerCounter[1]) > -5.0f)
            {
                trackSlot->isOccupiedByLocker = true;
        ...
        void PldFusion::slotTypeHandle(const FusionSlot::Ptr tSlot)
        {
            if (!isParking_)
            {
                ...
                {
                    if ((tSlot->width > 4.6) && (tSlot->width > tSlot->length))
                    {
                        tSlot->type = FusionSlot::Type::HORIZONTAL;
                    }
                    else
                    {
                        if ((tSlot->width > tSlot->length) && (tSlot->lines_[2]->couldBeingAsOpenLine == Fusion::CTrackLine::OpenLineStatus::occupiedByNeighborSlot) && (tSlot->width > 4.4))
                        {
                            tSlot->type = FusionSlot::Type::HORIZONTAL;
                        }
                        else
        ...
        void PldFusion::ParallelDepthHandle(const FusionSlot::Ptr slot)
        {
            ...
            slot->length = 0.5 * ((slot->corners[1].worldCorner - slot->corners[2].worldCorner).norm() + (slot->corners[3].worldCorner - slot->corners[0].worldCorner).norm());
        ...
        {
            if ((1.5f * slot->lines_[0]->getLength() > slot->lines_[1]->getLength()) || (1.5f * slot->lines_[0]->getLength() > slot->lines_[3]->getLength()))
            {
        ...
        {
            if ((fusionSlot->type != FusionSlot::Type::VERTICAL) && (fusionSlot->type != FusionSlot::Type::INCLINED))
            {
                return;
            }
            ...
            if ((fusionSlot->corners[0].isCornerAsTruePoint == true) && (fusionSlot->corners[1].isCornerAsTruePoint == true))
            {
                if (((fusionSlot->corners[2].isCornerAsTruePoint == false) && (fusionSlot->lines_[1]->getLength() < fusionSlot->lines_[1]->lastFrameLength)) || ((fusionSlot->corners[2].isCornerAsTruePoint == true) && (fusionSlot->lines_[1]->getLength() < fusionSlot->lines_[1]->lastFrameLength) && ((fusionSlot->lines_[1]->lastFrameLength - fusionSlot->lines_[1]->getLength()) > 1.5f)))
                {
                ...
                if (((fusionSlot->corners[3].isCornerAsTruePoint == false) && (fusionSlot->lines_[3]->getLength() < fusionSlot->lines_[3]->lastFrameLength)) || ((fusionSlot->corners[3].isCornerAsTruePoint == true) && (fusionSlot->lines_[3]->getLength() < fusionSlot->lines_[3]->lastFrameLength) && ((fusionSlot->lines_[3]->lastFrameLength - fusionSlot->lines_[3]->getLength()) > 1.5f)))
                {
            ...
            fusionSlot->length = 0.5 * ((fusionSlot->corners[1].worldCorner - fusionSlot->corners[2].worldCorner).norm() + (fusionSlot->corners[3].worldCorner - fusionSlot->corners[0].worldCorner).norm());
        }
        ...
        void PldFusion::AlignAjoinedSlots(std::list<FusionSlot::Ptr> &slots)
        {
            ...
            for (auto &slot : slots)
            {
                if ((slot->ajoin.rightSlot != nullptr && slot->ajoin.leftSlot == nullptr) || (slot->ajoin.rightSlot == nullptr && slot->ajoin.leftSlot == nullptr))
                {
        ...
        void PldFusion::MakeAlign(const std::vector<FusionSlot::Ptr> &ajoinSlots)
        {
            ...
            if ((angleSum < (double)(70.0f / 180.0f) * M_PI) || (angleSum > (double)(110.0f / 180.0f) * M_PI))
            {
                for (auto &slot : ajoinSlots)
                {
            ...
            const int targetID)
        {
            ...
            for (const auto &outSlot : mergedSlotsList)
            {
                if (outSlot->id == targetID)
                {
                    ...
                    if ((outSlot->alignInfo.pldSlot != nullptr) && (outSlot->alignInfo.pldSlot->sourceSlot != nullptr) && (outSlot->alignInfo.pldSlot->sourceSlot->relativeSlots.size() > 0))
                    {
