        ...
        for (int i = 0 ; i < int(cluster->pointsBase.size()) ; ++i) {
            for (int j = i + 1 ; j < int(cluster->pointsBase.size()) ; ++j) {
                const double dist = (cluster->pointsBase[i]->point -
                                     cluster->pointsBase[j]->point).norm();
                if (dist < maxDist) {
                    continue;
                }
                maxDist = dist;
                         ...
                         visualization_msgs::msg::MarkerArray& markers)
{
    ...
    const double centerX = fusionLane->param_.isInverse ?
                           fusionLane->param_.GetY((fusionLane->param_.start + fusionLane->param_.end) / 2) :
                           (fusionLane->param_.start + fusionLane->param_.end) / 2;

    const double centerY = fusionLane->param_.isInverse ?
                           (fusionLane->param_.start + fusionLane->param_.end) / 2 :
                           fusionLane->param_.GetY((fusionLane->param_.start + fusionLane->param_.end) / 2);
    ...
    const std::string output = fusionLane->param_.isInverse ?
                               "x= " + std::to_string(fusionLane->param_.k) + " y+ " +
