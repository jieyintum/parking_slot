...
namespace Fusion 
{
        ...
        const Type f_c_r)
    {
        ...
        if( ((f_a_r >= f_b_r) && (f_b_r >= f_c_r))
         || ((f_a_r <= f_b_r) && (f_b_r <= f_c_r)) )
        {
    ...
    template <class VectorType> inline VectorType getUnitVector(const VectorType& f_vec_v)
    {
    ...
    template <class Type> inline bool isZero(Type f_value)
    {
        return (std::fabs(f_value) < 0.000001f) ? true : false;
    }


    template <class Type> inline bool isNegative(Type f_value)
    {
        return (f_value < 0.0f) ? true:false;
    }

    template <class Type> inline bool isPositive(Type f_value)
    {
        return (f_value > 0.0f) ? true:false;
    }
